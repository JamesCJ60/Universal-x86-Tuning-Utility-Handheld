...
using static Universal_x86_Tuning_Utility_Handheld.Scripts.Misc.Display;
using static Universal_x86_Tuning_Utility_Handheld.Scripts.Misc.ScreenInterrogatory;
...
namespace Universal_x86_Tuning_Utility_Handheld.Scripts.Misc
{
    internal class Display
    {
        ...
        {
            uniqueResolutions = GetSupportedResolutions(targetDisplayName);
            uniqueRefreshRates = GetSupportedRefreshRates(targetDisplayName);

            uniqueResolutions = uniqueResolutions.Distinct().ToList();
        ...
        public static void ChangeDisplaySettings(string targetDisplayName, int newWidth, int newHeight, int newRefreshRate)
        {
            DEVMODE devMode = new DEVMODE();
            devMode.dmSize = (short)Marshal.SizeOf(devMode);
        ...
        public static void ApplySettings(string input, int newHz)
        {
            ...
            if (dimensions.Length == 2)
            {
                ...
                {
                    Display.ChangeDisplaySettings(Display.targetDisplayName, width, height, newHz);
                }
                else
                {
